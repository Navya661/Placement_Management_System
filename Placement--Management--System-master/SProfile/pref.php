...
if($USN !=''||$email !='')
{
	if($USN == $sun)
    {
    if($query = $connect->query("INSERT INTO `helloworld`.`basicdetails` ( `FirstName`, `LastName`, `USN`, `Mobile`, `Email`, `DOB`, `Sem`, `Branch`, `SSLC`, `PU/Dip`,`BE`,`Backlogs`, `HofBacklogs`, `DetainYears`, `Approve`)
...
if($USN !=''||$email !='')
{
	...
	$sql = $connect->query("SELECT * FROM `helloworld`.`basicdetails` WHERE `USN`='$USN'");
	if($sql->num_rows == 1)
	{
